/**
 * Kids catalogue — the seed data.
 *
 * Once the site is deployed, the live catalogue lives in D1 and is edited from
 * the dashboard; this file is imported twice more:
 *   - `worker/src/seed.ts` copies it into the database on first run
 *   - `src/context/CatalogContext.tsx` falls back to it when the API is not
 *     reachable, so `npm run dev` works on its own
 *
 * The photographs are the real product shots supplied in
 * `original products images/` (GEPS line-up). Prices and descriptions were
 * placeholders; winter pajamas and bodysuit packs now carry the client's
 * prices, and everything is editable from the dashboard.
 */

/** A category slug. Free-form, because categories are created in the dashboard. */
...
// Every tile is a real product photograph (`photo: true` crops it to fill).
export const categories: Category[] = [
  {
    ...
    nameAr: "بيجامات",
    nameEn: "Pajamas",
    image: "/images/products/blue-dragon-pajama-1.jpg",
  ...
  },
  {
    ...
    image: "/images/products/spiderman-summer-set-1.jpg",
  ...
  },
  {
    ...
    image: "/images/products/girl-tee-pack-unicorn-1.jpg",
...
export type Product = {
  ...
  colors: Swatch[];
  isNew?: boolean;
  /** Set from the dashboard; the seed treats everything as available. */
  inStock?: boolean;
  popularity: number;
  createdAt: string;
};
...
// Looking a product up, finding related ones and listing what is on sale all
// live on the live catalogue now — see `useCatalog()` in
// `src/context/CatalogContext.tsx`.

/** Fallback shipping destinations, used only when the API is unreachable. */
